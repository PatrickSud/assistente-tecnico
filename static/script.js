...
async function selectApp(appType) {
  selectedApp = appType

  // Update header based on software selection
  const subtitle = document.getElementById('app-subtitle')
  if (subtitle) {
  ...
  // Special handling for Agente: skip preparation, kill processes immediately
  if (appType === 'agente') {
    try {
      ...
      if (data.success) {
        navigationHistory.push('step-version')
  ...
  // Update title and description based on app type for other apps
  ...
  if (appType === 'buscanfe') {
    ...
    // Fallback or generic preparation (though agente is now handled above)
    if (prepTitle) prepTitle.textContent = 'Preparação'
    if (prepDescription) {
      prepDescription.style.display = 'block'
      prepDescription.innerHTML =
        'Vamos preparar o ambiente para a atualização.'
    }
  }
  ...
  if (appType === 'dominio') {
    ...
    document.querySelector(
      'input[name="operationType"][value="update"]'
    ).checked = true
    toggleOperationMode()
    ...
    document.getElementById('dominio-update-area').style.display = 'none'
...
async function fetchDominioUpdateInfo() {
  try {
    ...
    if (data.success) {
      ...
      const adjustmentSpan = document.getElementById(
        'dominio-adjustment-version'
      )
...
async function fetchDominioVersions() {
  ...
  const moreContainer = document.getElementById(
    'dominio-all-versions-container'
  )
...
async function fetchDominioUpdateVersions() {
  ...
  const moreContainer = document.getElementById(
    'dominio-update-all-versions-container'
  )
  ...
  try {
    ...
    if (data.success && data.versions.length > 0) {
      ...
      grid.innerHTML =
        '<div style="grid-column: 1 / -1; text-align: center; color: var(--text-muted); font-size: 0.9rem;">Nenhuma versão de atualização encontrada.</div>'
    }
  } catch (e) {
...
function showAdjustmentsSelection(version, adjustments) {
  ...
  document.getElementById('dominio-update-versions-container').style.display =
    'none'

  // Configurar container de ajustes
  const container = document.getElementById(
    'dominio-update-adjustments-container'
  )
...
function startPollingStatus() {
  ...
  pollInterval = setInterval(async () => {
    try {
      ...
      document.getElementById('dl-size').innerText =
        state.downloaded_mb.toFixed(2)

      const speedEl = document.getElementById('dl-speed')
      if (speedEl && state.speed_mbps !== undefined) {
        speedEl.innerText = state.speed_mbps.toFixed(1)
      }
      ...
      if (state.status === 'download_complete') {
        ...
        setTimeout(async () => {
          ...
          if (selectedApp === 'agente') {
            document.getElementById('install-check-area').style.display =
              'block'
            document.getElementById('dominio-finish-area').style.display =
              'none'
          } else {
...
async function verifyInstallation() {
  try {
    ...
    if (data.success) {
      ...
      alert(
        'O Agente ainda não foi encontrado. Certifique-se de concluir a instalação e tente novamente.'
      )
    }
  } catch (e) {
...
async function exitApp() {
  try {
    ...
    try {
      ...
      console.warn(
        'Erro ao contatar servidor para sair (pode já estar fechado):',
        err
      )
    }
    ...
    document.body.innerHTML =
      "<div style='display:flex;justify-content:center;align-items:center;height:100vh;flex-direction:column;font-family:sans-serif;text-align:center;'><h1>Aplicação Encerrada</h1><p>Você pode fechar esta guia agora.</p></div>"
  } catch (e) {
